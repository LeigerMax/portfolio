    ...
    <div className="w-full max-w-6xl px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((project, idx) => (
          <div
            key={idx}
            className="group bg-black/50 backdrop-blur-md border border-white/10 rounded-2xl overflow-hidden hover:border-orange-500/40 transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_20px_60px_-15px_rgba(249,115,22,0.15)] flex flex-col"
          >
            {/* Image du projet */}
            ...
            <div className="p-6 flex flex-col flex-1">
              {/* Titre */}
              <h3 className="text-xl font-black text-white uppercase tracking-tight mb-2 group-hover:text-orange-400 transition-colors">
                {project.title}
              </h3>
              ...
              <div className="flex flex-wrap gap-1.5 mb-3">
                {project.tech.map((t: string) => (
                  <span key={t} className="text-[10px] uppercase font-bold tracking-wider text-orange-400/80 bg-orange-500/10 border border-orange-500/20 px-2 py-0.5 rounded-full">
                    {t}
                  </span>
                ))}
              </div>
              ...
              <button
                ...
                className="w-full py-2.5 bg-white/5 border border-white/10 text-white text-sm font-bold uppercase tracking-wider rounded-xl hover:bg-orange-500 hover:border-orange-500 hover:text-black transition-all duration-300 cursor-pointer"
              >
                Découvrir →
              </button>
            </div>
            ...
            <div className="p-12 -mt-12 relative">
              ...
              <div className="flex flex-wrap gap-2 mb-8">
                {selectedProject.tech.map((t: string) => (
                  <span key={t} className="px-3 py-1 bg-orange-500/10 text-orange-400 text-xs font-bold uppercase tracking-widest rounded-full border border-orange-500/20">
                    {t}
                  </span>
                ))}
              </div>
